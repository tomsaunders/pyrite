...
abstract class FlightGroupBase extends PyriteBase implements Byteable {
	...
	public function __construct($hex){
		$this->hex = $hex;
		$offset = 0;
		$this->Name = $this->getChar($hex, 12, 0x000);
		$this->Pilot = $this->getChar($hex, 12, 0x00C);
		$this->Cargo = $this->getChar($hex, 12, 0x018);
		$this->SpecialCargo = $this->getChar($hex, 12, 0x024);
	...
	public function __debugInfo() {
        ...
        protected function getCraftTypeLabel() {
            return isset($this->CraftType) ? Constants::$CRAFTTYPE[$this->CraftType] : 'Unknown';
        }


        protected function getStatusLabel() {
            return isset($this->Status) ? Constants::$STATUS[$this->Status] : 'Unknown';
        }


        protected function getWarheadLabel() {
            return isset($this->Warhead) ? Constants::$WARHEAD[$this->Warhead] : 'Unknown';
        }


        protected function getBeamLabel() {
            return isset($this->Beam) ? Constants::$BEAM[$this->Beam] : 'Unknown';
        }


        protected function getGroupAILabel() {
            return isset($this->GroupAI) ? Constants::$GROUPAI[$this->GroupAI] : 'Unknown';
        }


        protected function getMarkingsLabel() {
            return isset($this->Markings) ? Constants::$MARKINGS[$this->Markings] : 'Unknown';
        }


        protected function getFormationLabel() {
            return isset($this->Formation) ? Constants::$FORMATION[$this->Formation] : 'Unknown';
        }


        protected function getArrivalDifficultyLabel() {
            return isset($this->ArrivalDifficulty) ? Constants::$ARRIVALDIFFICULTY[$this->ArrivalDifficulty] : 'Unknown';
        }


        protected function getAbortTriggerLabel() {
            return isset($this->AbortTrigger) ? Constants::$ABORTTRIGGER[$this->AbortTrigger] : 'Unknown';
        }
